...
function positional_spec(p, gs)
    9 <= length(p) <= 12 || error(
        "Expected <gpus> <cpus> <name> <T> <N> <M> <iter> <warmup> <trial> [fusion] [check_correctness] [correctness_iter]"
    )
...
function plan_manifest(runs, budget, raw)
    ...
    versions = Dict(
        ...
        info in values(Pkg.dependencies()) if info.version !== nothing
    )
    ...
    if isempty(opts.positional)
        for T in unique(results_subdir(r.spec) for r in runs)
            try
                ...
                launch(
                    `$(Base.julia_cmd()) --project=$root $plotter $results --config=$(opts.config) --out=$out --suffix=$suffix`
                )
            catch e
                failed = true
