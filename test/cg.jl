@testset "CG periodic checks and harness integration" begin
    for B in (ConjugateGradientBenchmark, ConjugateGradientAccelerated),
    ...
    @test Set(r.model for r in runs)==Set((:cunumeric, :cupynumeric, :cudajl, :jacc, :dagger))
    @test Set((r.spec.name, r.model) for r in runs)==Set([
        ("cg", :cunumeric), ("cg", :cupynumeric), ("cg", :cudajl), ("cg", :jacc), ("cg", :dagger),
        ("cg_plain", :cunumeric)])
    ...
    request=WorkerRequest(
        1, 1, "cg", "Float64", 17, 1, 1, 0, 1, true, 5, 0.0, Dict(:check_every=>4, :max_iter=>40)
    )
    worker=parse_model_worker_args(collect(common_worker_args(request)))
    @test worker.kwargs==request.kwargs
    @test isempty(
        parse_model_worker_args(
            collect(
                common_worker_args(
                    WorkerRequest(1, 1, "cg", "Float64", 17, 1, 1, 0, 1, true, 5, 0.0)
                ),
            ),
        ).kwargs,
    )
    ...
    mktemp() do path, io
        ...
        other=deepcopy(raw["cg"][1])
        other["kwargs"]["check_every"]=4
        push!(raw["cg"], other)
        TOML.print(io, raw)
        close(io)
    ...
    # "cg" is the default accelerated solve; "cg_plain" is cuNumeric-only.
    @test !supports_benchmark(JACCModel(), "cg_plain")
    @test supports_run(JACCModel(), "cg", 2)
    @test_throws ErrorException memory_estimate(
        ConjugateGradientAccelerated{Float64}(; N=17), MemoryContext(; model=:jacc, gpus=2))
    @test peak_bytes(
        memory_estimate(
            ConjugateGradientAccelerated{Float64}(; N=16), MemoryContext(; model=:jacc, gpus=2)),
    ) > 0
    # Every model runs the default "cg"; only cuNumeric runs "cg_plain".
    @test supports_benchmark(CUDAJLModel(), "cg")
    @test supports_benchmark(CUDAJLModel(), "cg_plain")
