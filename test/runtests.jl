...
const MONTECARLO_CONFIG = joinpath(@__DIR__, "..", "configs", "multi_gpu", "montecarlo.toml")
...
@testset "Execution model registry and isolation" begin
    ...
    for (environment, expected, excluded) in (
        ("jacc", "JACC", "Dagger"), ("dagger", "Dagger", "JACC")
    )
        directory = joinpath(@__DIR__, "..", "environments", environment)
        # Fresh standalone clones have no manifests. Check declared isolation
        # always, and resolved transitive dependencies when instantiated (CI).
        files = [joinpath(directory, "Project.toml")]
        manifest = joinpath(directory, "Manifest.toml")
        isfile(manifest) && push!(files, manifest)
        for file in files
            deps = TOML.parsefile(file)["deps"]
            @test haskey(deps, expected)
            @test !haskey(deps, excluded)
            @test !haskey(deps, "cuNumeric")
        end
    end
...
@testset "Memory dispatch matrix" begin
    for T in (Float32, Float64),
        ...
        for (name, B) in BENCHMARKS
            ...
            B <: Union{NASEmbarrassinglyParallel,NASFourierTransform,NASMultiGrid} &&
                continue
            m = if B <: AbstractDMD
                16
